...
if [ "$1" = "test" ]; then
    ...
    assert 15 "{a = 10; a + 5; }"
    assert 15 "{a = 10; a += 5; return a; 45; }"
    assert 5 "{a = 10; a -= 5; return a; 45; }"
    assert 50 "{a = 10; a \*= 5; return a; 56; }"
    assert 2 "{a = 10; a /= 5; return a; 45; }"
    assert 2 "{a = 10; a %= 4; return a; 34; }"
    assert 2 "{a = 10; a &= 3; return a; 45; }"
    assert 11 "{a = 10; a |= 3; return a; 465; }"
    assert 9 "{a = 10; a ^= 3; return a; 123; }"
    assert 80 "{a = 10; a <<= 3; return a; 123; }"
    assert 2 "{a = 10; a >>= 2; return a; 123; }"
    assert 14 "{a = 10, b = 4; a + b; }"
    assert 2 "{a = 10, b = 4; a - (b \* 2); }"
    assert 10 "{a = b = 5; a + b; }"
    assert 27 "{a = b = c = 5; c = 2; a \* b + c; }"
    ...
    assert 16 "{ a = 4; if (a == 4) { return 4 \* 4; } else return 4 + (8 - 4); }"
    assert 5 "{ i = 0; while (i < 5) i += 1; return i; }"
    assert 5 "{ i = 0; do i += 1; while (i < 5); return i; }"
    assert 5 "{for (i = 0; i < 10; i += 1) { if (i == 5) return i; } }"
    assert 10 "{ i = 0; for (; i < 10; i += 1); return i; }"
    assert 8 "{ i = 0; for (;i < 8;) i += 1; return i; }"
    assert 4 "{ i = 0; for (;;) { i += 1; if (i == 4) return i; } return 45; }"
    assert 2 "{ i = 7; while (i > 2) { i = i - 1; } return i; }"
    assert 16 "{ x = 4, y = 4, q = 0; for (i = 0; i < y; i += 1)\n for (j = 0; j < x; j += 1)\n q += 1; return q; }"
